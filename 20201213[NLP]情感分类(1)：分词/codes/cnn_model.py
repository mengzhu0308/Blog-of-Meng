...
def res_block(x, filters, kernel_size=3, strides=1):
    shortcut = x
    x = Conv1D(filters * 2, kernel_size, strides=strides, padding='same', kernel_initializer='he_normal')(x)
    x = Lambda(lambda x_arg: x_arg[..., :filters] * K.sigmoid(x_arg[..., filters:]))(x)

    x = Conv1D(filters * 2, kernel_size, padding='same', kernel_initializer='he_normal')(x)
    x = Lambda(lambda x_arg: x_arg[..., :filters] * K.sigmoid(x_arg[..., filters:]))(x)
...
def CNN_Model(x, vocab_size=27795, hidden_dim=256, max_length=40, num_classes=4):
    x = Embedding(vocab_size + 1, hidden_dim, input_length=max_length)(x)
    x = Conv1D(32 * 2, 3, strides=2, padding='same', kernel_initializer='he_normal')(x)
